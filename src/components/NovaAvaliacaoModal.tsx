...
export default function NovaAvaliacaoModal({ isOpen, onClose }: NovaAvaliacaoModalProps) {
  // Dados Básicos do Cliente (caso não tenha cadastro)
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-xl max-w-7xl w-full max-h-[95vh] overflow-y-auto my-8">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center z-10">
          <div>
            <h2 className="text-2xl font-bold text-amber-700">📝 Nova Avaliação</h2>
            <p className="text-sm text-gray-600">Avaliação completa para cliente novo ou sem pré-cadastro</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          {/* Dados Básicos do Cliente (Opcional - apenas se novo) */}
          <div className="border-2 border-blue-200 rounded-xl p-6 bg-blue-50">
            <h3 className="text-lg font-bold text-blue-700 mb-4">👤 Dados Básicos do Cliente (Opcional)</h3>
            <p className="text-sm text-blue-600 mb-4">
              Se o cliente já está cadastrado, pode pular esta seção
            </p>
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input 
                  ...
                  onChange={(e) => setDadosBasicos({...dadosBasicos, idade: e.target.value})}
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:outline-none text-sm bg-white"
                />
              </div>
              <div>
                ...
                <input 
                  type="text"
                  value={dadosBasicos.altura}
                  onChange={(e) => setDadosBasicos({...dadosBasicos, altura: e.target.value})}
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:outline-none text-sm bg-white"
                />
              </div>
              <div>
                ...
                <input 
                  type="text"
                  value={dadosBasicos.peso_atual}
                  onChange={(e) => setDadosBasicos({...dadosBasicos, peso_atual: e.target.value})}
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:outline-none text-sm bg-white"
                />
              </div>
              <div>
                ...
                <input 
                  type="text"
                  value={dadosBasicos.peso_desejado}
                  onChange={(e) => setDadosBasicos({...dadosBasicos, peso_desejado: e.target.value})}
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:outline-none text-sm bg-white"
                />
              </div>
              <div>
          ...
          <div className="sticky bottom-0 bg-white border-t border-gray-200 px-6 py-4 flex justify-end gap-4">
            ...
            <button
              type="submit"
              className="px-6 py-3 bg-gradient-to-r from-amber-600 to-amber-700 text-white rounded-lg hover:scale-105 shadow-lg font-semibold"
            >
